...
# CSS Injection - FORMAL MAROON, CREAM & WHITE THEME
st.markdown("""
    <style>
    /* ── Colour Palette ──────────────────────────────────────────────
       Maroon  : #6B0F1A  (deep)  |  #8B1A2A  (mid)  |  #A52535 (accent)
       Cream   : #F5ECD7  (base)  |  #EDE0C4  (mid)  |  #FDF8F0 (light)
       White   : #FFFFFF
       Text    : #2C1810  (dark brown-black)
    ──────────────────────────────────────────────────────────────── */
    ...
    /* 2. ROOT — force light background & dark text everywhere */
    html, body,
    .main,
    [data-testid="stAppViewContainer"],
    [data-testid="stAppViewContainer"] > section,
    [data-testid="block-container"] {
        background: linear-gradient(160deg, #FDF8F0 0%, #F5ECD7 100%) !important;
    ...
    /* 3. Sidebar — white with maroon accents */
    [data-testid="stSidebar"],
    [data-testid="stSidebar"] > div {
        background-color: #FFFFFF !important;
        border-right: 3px solid #8B1A2A !important;
    }
    ...
    [data-testid="stFileUploader"] {
        ...
        border: 2px dashed #8B1A2A !important;
    ...
    [data-testid="stFileUploader"] button {
        background: linear-gradient(135deg, #6B0F1A 0%, #A52535 100%) !important;
        border: none !important;
        border-radius: 8px !important;
        padding: 8px 16px !important;
        box-shadow: 0 4px 8px rgba(107, 15, 26, 0.25) !important;
    }
    [data-testid="stFileUploader"] button * {
    ...
    [data-testid="stFileUploader"] text { fill: #6B0F1A !important; }
    [data-testid="stFileUploader"] small,
    [data-testid="stFileUploader"] span { color: #5A3A2A !important; font-weight: 500 !important; }

    /* 5. Main headings */
    .main h1, h1 {
        color: #6B0F1A !important;
        -webkit-text-fill-color: #6B0F1A !important;
    ...
    .main h3, h3 {
        ...
        -webkit-text-fill-color: #8B1A2A !important;
        font-family: 'Playfair Display', serif !important;
        font-weight: 700 !important;
    }
    .main p, p { color: #4A2C20 !important; font-weight: 500; }

    /* 6. Metric cards */
    [data-testid="stMetric"] {
        background: #FFFFFF !important;
        border-radius: 10px !important;
        border-left: 5px solid #8B1A2A !important;
        border-top: 1px solid #EDE0C4 !important;
        border-right: 1px solid #EDE0C4 !important;
        border-bottom: 1px solid #EDE0C4 !important;
        padding: 12px 16px !important;
        box-shadow: 0 4px 12px rgba(107, 15, 26, 0.08) !important;
    }
    ...
    .stButton>button:hover {
        ...
        background: linear-gradient(135deg, #8B1A2A 0%, #6B0F1A 100%) !important;
        color: #FFFFFF !important;
    }
    ...
    [data-testid="stDataFrame"] > div {
        background-color: #FFFFFF !important;
        border: 2px solid #EDE0C4 !important;
        border-radius: 10px !important;
        box-shadow: 0 2px 8px rgba(107, 15, 26, 0.05) !important;
    }
    /* Table cells: force light background & dark text */
    .stDataFrame iframe,
    [data-testid="stDataFrame"] iframe { background: #FFFFFF !important; }

    /* Glide table overrides (Streamlit internal) */
    .glideDataEditor, .gdg-scrolling-shadow, .dvn-scroller { 
        background: #FFFFFF !important; 
    }
    .gdg-style { 
    ...
    button[data-baseweb="tab"] {
        font-weight: 700 !important;
        color: #6B4035 !important;
        font-size: 0.92rem !important;
        background: transparent !important;
    }
    button[data-baseweb="tab"][aria-selected="true"] {
        color: #6B0F1A !important;
        border-bottom-color: #6B0F1A !important;
    }
    [data-testid="stTabs"] [role="tablist"] {
        background: transparent !important;
        border-bottom: 2px solid #EDE0C4 !important;
    }

    /* 10. Dividers */
    hr { margin: 1.5rem 0 !important; border-color: #D9C9B0 !important; }
    ...
    [data-testid="stAlert"] span { color: #2C1810 !important; }

    /* 12. Slider */
    [data-testid="stSlider"] > div > div > div > div {
        background: #8B1A2A !important;
    }
    [data-testid="stSlider"] span { color: #3D1A12 !important; }
    ...
    [data-testid="stNumberInput"] span { color: #2C1810 !important; }
    ...
    [data-baseweb="select"] div { color: #2C1810 !important; }
    [data-baseweb="popover"] ul,
    [data-baseweb="menu"] { background: #FFFFFF !important; }
    [data-baseweb="menu"] li { color: #2C1810 !important; }
    [data-baseweb="menu"] li:hover { background: #F5ECD7 !important; }

    /* 15. Radio buttons */
    [data-testid="stRadio"] label { color: #3D1A12 !important; }
    [data-testid="stRadio"] span { color: #3D1A12 !important; }
    ...
    [data-testid="stExpander"] summary p { color: #6B0F1A !important; font-weight: 600 !important; }

    /* 18. Caption */
    [data-testid="stCaptionContainer"] p { color: #7A5C50 !important; }
    ...
    [data-testid="stDownloadButton"] button span { color: #6B0F1A !important; }

    /* 20. Subheader special — bold maroon with bottom border */
    ...
    /* 22. Markdown text & code */
    .stMarkdown p, .stMarkdown span, .stMarkdown li { color: #2C1810 !important; }
    .stMarkdown strong { color: #6B0F1A !important; }
    code, pre { background: #FDF8F0 !important; color: #6B0F1A !important; }
...
def plot_actual_forecast(periods, actual, forecast, title):
    ...
    fig.add_trace(go.Scatter(x=periods, y=actual, mode="lines+markers", name="Actual", line=dict(color='#6B0F1A', width=3)))
    # Forecast Color: Bright Pink/Neon Red
    fig.add_trace(go.Scatter(x=periods, y=forecast, mode="lines+markers", name="Forecast", line=dict(color='#A52535', width=3)))
...
def plot_future_forecast_with_ci(all_periods, actual_values, future_periods, future_forecast, residual_std=0):
    ...
    fig.add_trace(go.Scatter(x=all_periods, y=actual_values, mode="lines+markers", name="Historical Actual", line=dict(color='#6B0F1A', width=3)))
    ...
    fig.add_trace(go.Scatter(x=future_periods, y=future_forecast, mode="lines+markers", name="Main Projection", line=dict(color='#A52535', width=3, dash='dash')))
...
def convert_all_to_excel(comparison_df, best_method_name, future_labels, future_forecast):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        header_format = workbook.add_format({
            'bold': True, 'bg_color': '#6B0F1A', 'font_color': '#FFFFFF', 
...
def plot_all_methods_comparison(test_periods, test, details_dict):
    ...
    colors = [
        "#6B0F1A","#A52535","#C07850","#8B5E3C","#D4A05A",
        "#06B6D4","#8B5CF6","#F97316","#84CC16","#64748B"
    ]
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=test_periods, y=test, mode="lines+markers",
        name="Actual", line=dict(color="#2C1810", width=4),
...
def plot_residual_analysis(test_periods, residuals):
    ...
    fig_time.add_trace(go.Scatter(
        ...
        name="Residual", line=dict(color="#6B0F1A", width=2),
        marker=dict(size=6)
    ))
    ...
    fig_hist.add_trace(go.Histogram(
        ...
        marker_color="#8B1A2A", opacity=0.8,
...
def plot_decomposition(values, period_labels, seasonal_periods=12):
    ...
    fig = go.Figure()
    colors_decomp = ["#6B0F1A", "#8B1A2A", "#C07850", "#A52535"]
    components = [
        ("Original", values),
        ("Trend", trend),
        ("Seasonal", seasonal),
...
def convert_full_comparison_excel(comparison_df, details_dict, future_labels, future_forecast, best_method):
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        workbook = writer.book
        header_fmt = workbook.add_format({"bold": True, "bg_color": "#6B0F1A", "font_color": "#FFFFFF", "border": 1, "align": "center", "valign": "vcenter"})
...
st.markdown(
    ...
    <div style="display:flex;align-items:center;gap:18px;margin-bottom:4px;">
        ...
        <div>
            <h1 style="margin:0;background:linear-gradient(135deg,#6B0F1A 0%,#A52535 100%);
...
if uploaded_file is None:
    ...
    st.markdown(
        """
        <div style="background:linear-gradient(135deg,#FDF8F0 0%,#F5ECD7 100%);border:1.5px solid #C9A882;
            ...
            <span style="color:#4A5568;font-weight:500;font-size:0.95rem;">
                ...
                <a href="https://docs.google.com/spreadsheets/d/1DVghuZxRV-Xj269UMerp_NLRkCuLys5j/edit?usp=sharing&ouid=104043343223917321673&rtpof=true&sd=true"
                   target="_blank"
                   style="color:#6B0F1A;font-weight:700;text-decoration:none;background:#EDE0C4;
                          padding:4px 12px;border-radius:6px;border:1px solid #C9A882;">
...
st.markdown(
    f"""<div style="background:linear-gradient(135deg,#FDF8F0 0%,#F5ECD7 100%);border-left:4px solid #6B0F1A;
        border-radius:8px;padding:10px 16px;margin-bottom:12px;display:flex;align-items:center;gap:8px;">
        <span style="font-size:1.2rem;">📂</span>
        <span style="color:#2C1810;font-weight:600;font-size:0.95rem;">
...
with tab_data:
    ...
    with c_roll:
        ...
        roll_fig = go.Figure()
        roll_fig.add_trace(go.Scatter(x=roll_df["Period"], y=roll_df["Actual"], name="Actual", mode="lines", line=dict(color="#6B0F1A")))
...
with tab_grafik:
    ...
    history_fig.add_trace(go.Scatter(x=period_labels, y=values, mode="lines+markers", name="Actual Values", line=dict(color="#8B1A2A", width=3)))
